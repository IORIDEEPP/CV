...
digitDatasetPath = fullfile('C:\Users\Francis\OneDrive - City, University of London\MSc_Data_Science_at_City_University\Course\INM460 - Computer Vision\CW\ComputerVision_CW','Faces_DataSource_Emotions');
...
label_emotion = countEachLabel(digitData);
...
minSetCount = min([label_emotion.Count]);
% minSetCount = 256;
...
CV_net_CNN_face_emotion = trainNetwork(training ,layers, options);
disp('net Train completed!');

%% Classify Validation Images and Compute Accuracy
predictedLabelsTest = classify(CV_net_CNN_face_emotion,test);
...
predictedLabelsTrain = classify(CV_net_CNN_face_emotion,training);
...
%     X = rgb2gray(X);
    ...
    Y_pred = predict(CV_net_CNN_face_emotion,X);
...
% 
    if I > 0.90
        ...
        label_folder = char(label_emotion{J,1});
...
save CV_net_CNN_face_emotion;
disp('Save completed');

%% load
cd('C:\Users\Francis\OneDrive - City, University of London\MSc_Data_Science_at_City_University\Course\INM460 - Computer Vision\CW\ComputerVision_CW\Models');
load CV_net_CNN_face_emotion;
disp('Load completed');
...
%         disp(['Face num: ', num2str(i)]);
        ...
        Y_pred = predict(CV_net_CNN_face_emotion,subIm);
%         disp('Classification completed!!');
...
%         outputFileName = fullfile(output_folder, ['Face_' num2str(count_face) '.jpg']);
